...
namespace VRTweaks.VolumeCloudPatches
{
    ...
    class RenderFrameReplacement
    {
        ...
        private static void EnsureHaltonSequence(ref VolumeCloudRenderer.HaltonSequence haltonSequence)
        {
            if (haltonSequence == null)
            {
                haltonSequence = new VolumeCloudRenderer.HaltonSequence()
                {
                    radix = 3
                };
            }
        ...
        private static void SetupHeroCloud(VolumeCloudRenderer instance, Material mat)
        {
        ...
        static bool Prefix(VolumeCloudRenderer __instance, RenderTexture source, RenderTexture destination, ref float ___updateTimer, ref int ___forceOutOfBound, ref bool ___qualityChanged,
            ref Material ___blitMat, ref RenderTexture ___lowresBuffer, ref int[,] ___offset, ref Camera ___mcam, ref VolumeCloudRenderer.HaltonSequence ___haltonSequence)
        {
            ...
            else
            {
                ...
                ___blitMat.SetVector(ShaderPropertyID._CloudsProjectionExtents, ___mcam.GetProjectionExtents(activeEye,
                        single * (1 << (VolumeCloudRenderer.downSample & 31)), single1 * (1 << (VolumeCloudRenderer.downSample & 31))));
